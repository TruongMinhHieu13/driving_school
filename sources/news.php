...
if ($id != '') {
    /* Lấy bài viết detail */
    $rowDetail = $d->rawQueryOne("select id, view, date_created, id_list, id_cat, id_item, id_sub, type, name$lang, slugvi, slugen, desc$lang, desc2$lang, content$lang, photo, options from #_news where id = ? and type = ? and find_in_set('hienthi',status) limit 0,1", array($id, $type));
    ...
    /* Lấy tags */
    $newsTags = $d->rawQuery("select id_tags from #_news_tags where id_parent = ?", array($rowDetail['id']));
    $newsTags = (!empty($newsTags)) ? $func->joinCols($newsTags, 'id_tags') : array();

    if (!empty($newsTags)) {
        $rowTags = $d->rawQuery("select id, name$lang, slugvi, slugen from #_tags where type='" . $type . "' and id in ($newsTags) and find_in_set('hienthi',status) order by numb,id desc");
    }
    ...
    /* Lấy hình ảnh con */
    $rowDetailPhoto = $d->rawQuery("select photo from #_gallery where id_parent = ? and com='news' and type = ? and kind='man' and val = ? and find_in_set('hienthi',status) order by numb,id desc", array($rowDetail['id'], $type, $type));
    ...
    /* Lấy tin tức cấp 1 */
    $newsList = $d->rawQuery("select id, name$lang, slugvi, slugen from #_news_list where type = 'tin-tuc' order by numb,id desc ");
