...
/* Array folders */
$upload_const = 'upload';
$array_folder = array('upload', 'thumbs', 'watermark');
...
foreach ($array_folder as $vfolder) {
    if (!file_exists(ROOT . $vfolder)) {
        mkdir(ROOT . $vfolder, 0777, true);
        chmod(ROOT . $vfolder, 0777);
    }
}
...
if (file_exists(ROOT . $upload_const) && $array_const) {
    ...
    foreach ($array_folder as $vfolder) {
        $path_htaccess = ROOT . $vfolder . '/.htaccess';
