...
class Functions
{
    ...
    public function ConvertFanpageToMessenger($url = '')
    {
        if (strpos($url, 'facebook.com') !== false) {
            $url = rtrim(str_replace(['https://', 'www.'], '', $url), '/');
    ...
    public function getCondition($data, $column = '', $where = '', $type = true) 
    { 
        if($where != ' ') $where .= ' or ';
        if($type == false){
            foreach($data as $k => $v){
                $where .= ($k == 0 ? '' : ' or') ." ".$column." = '".$v['MA_KHOA_HOC']."'";
            }
        } else {
            foreach($data as $k => $v){
                if($column == 'HO_VA_TEN'){
                    $where .= ($k == 0 ? '' : ' or') ." ".$column." LIKE '%".$v."%'";
                } else {
                    $where .= ($k == 0 ? '' : ' or') ." ".$column." = '".$v."'";
                }
            }
        }
        return $where;
    ...
    public function getYoutubeShorts($str)
    {
        $char = 'shorts/';
        $pos = strpos($str, $char);
        if ($pos != false) {
    ...
    public function checkProductSale($data = array(), $idColor = 0, $idSize = 0)
    {
        foreach ($data as $item) {
            if ($idColor == $item['id_color'] && $idSize == $item['id_size']) {
                return $item['id'];
            }
        }
        return 0;
    ...
    public function writeJson($data = array())
    {
        ...
        if ($data['type'] == 'product_sale') {
            $variation = $this->d->rawQuery("select * from #_product_sale where id_parent = ?", array($data['id_product']));

            /* Put data */
            if (!empty($variation)) {
                $this->putJson('variation-' . $data['id_product'] . '.json', $variation);
            }
        } else if ($data['type'] == 'create_all_product_sale') {
            $product = $this->d->rawQuery("select id from #_product where type = 'san-pham'", array());
            foreach ($product as $key => $value) {
                $variation = $this->d->rawQuery("select * from #_product_sale where id_parent = ?", array($value['id']));

                /* Put data */
                if (!empty($variation)) {
                    $this->putJson('variation-' . $value['id'] . '.json', $variation);
                }
            }
            ...
            if (!empty($data['idCity']) && !empty($data['idDistrict'])) {
                ...
                $districts = $this->d->rawQuery("select id_city, id from #_district where find_in_set('hienthi',status)");
                if (!empty($districts)) {
                    $result = array();
    ...
    public function checkColor($id_pro, $id_color){
        global $configBase; 
        $jsonPath = $configBase.JSONS . 'variation-' . $id_pro . '.json?v=' . time();
        $jsonData = file_get_contents($jsonPath);
        $data = json_decode($jsonData);
        if ($data !== null) {
            foreach ($data as $item) {
                if($item->id_color == $id_color && $item->status > 0){
                    return true;
                }
            }
        }   
    ...
    public function checkSize($id_pro, $id_size){
        global $configBase; 
        $jsonPath = $configBase.JSONS . 'variation-' . $id_pro . '.json?v=' . time();
        $jsonData = file_get_contents($jsonPath);
        $data = json_decode($jsonData);
        if ($data !== null) {
            foreach ($data as $item) {
                if($item->id_size == $id_size && $item->status > 0){
                    return true;
                }
            }
        }   
...
*/
    ...
    public function makeDate2($time = 0, $dot = '/', $lang = 'vi', $f = true)
    {
        $str = ($lang == 'vi') ? date("d{$dot}m{$dot}Y", $time) : date("m{$dot}d{$dot}Y", $time);
    ...
    public function uploadImage($file = '', $extension = '', $folder = '', $newname = '')
    {
        ...
        if (isset($_FILES[$file]) && !$_FILES[$file]['error']) {
            ...
            $MaxSize = $MaxSize[0];
            if ($_FILES[$file]['size'] > (int)$MaxSize * 1048576) {
    ...
    public function getColDetail($cols = '', $table = '', $id = 0)
    {
        $row = array();
        ...
        if (!empty($row)) return $row[$cols]; else return '';
    ...
    /* Join column */
    public function joinColsArray($array = null, $column = null)
    {
        $str = '';
        $arrayTemp = array();
    ...
    {
        global $type, $dataVariationColor;
        ...
        if ($id) {
            ...
            $dataVariationColor = json_encode($temps);
        }

        $row_color = $this->d->rawQuery("select namevi, id from #_color where type = ? order by numb,id desc", array($type));

        $str = '<select id="dataColor" name="dataColor[]" class="select multiselect-notall" multiple="multiple" >';
    ...
    public function getSize($id = 0)
    {
        ...
        if ($id) {
            ...
            $temps = (!empty($temps)) ? explode(",", $temps) : array();
            $dataVariationSize = json_encode($temps);
        }

        $row_size = $this->d->rawQuery("select namevi, id from #_size where type = ? order by numb,id desc", array($type));

        $str = '<select id="dataSize" name="dataSize[]" class="select multiselect-notall" multiple="multiple" >';
    ...
    /* End Product Seen */
    public function getNews($news, $class='')
    {
        global $lang,$sluglang , $type;
        if($news){
            foreach ($news as $k => $v) {              
                ?>      
                <div class="<?=$class?>">
                    <div class="news-items">
                        <div class="pic-news text-center">
                              <a class="scale-img" href="<?=$v[$sluglang]?>" title="<?= $v['name' . $lang] ?>">
                                    <img class="w-100" onerror="this.src='<?= THUMBS ?>/385x260x1/assets/images/noimage.png';" src="<?= THUMBS ?>/385x260x1/<?= UPLOAD_NEWS_L . $v['photo'] ?>" alt="<?= $v['name' . $lang] ?>" title="<?= $v['name' . $lang] ?>" />
                                </a>
                            
                        </div>
                        <div class="info-news">
                            <h3 class="news-name">
                                <a class="text-split-2" href="<?= $v[$sluglang] ?>" title="<?= $v['name' . $lang] ?>"><?= $v['name' . $lang] ?></a>
                            </h3>
                            <p class="news-desc text-split"><?= $v['desc' . $lang] ?></p>
                            <div class="text-center"><a class="see-news" href="<?= $v[$sluglang] ?>" title="<?= $v['name' . $lang] ?>">Xem thêm</a></div>
                        </div>
                    </div>
                </div>
            <?php }
    ...
    public function getService($news, $class='')
    {
        global $lang,$sluglang , $type;
        if($news){
            foreach ($news as $k => $v) {              
                ...
                <div class="<?=$class?>">
                    <div class="service-items">
                        <div class="pic-service text-center">
                              <a class="scale-img" href="<?=$v[$sluglang]?>" title="<?= $v['name' . $lang] ?>">
                                    <img class="w-100" onerror="this.src='<?= THUMBS ?>/385x330x1/assets/images/noimage.png';" src="<?= THUMBS ?>/385x330x1/<?= UPLOAD_NEWS_L . $v['photo'] ?>" alt="<?= $v['name' . $lang] ?>" title="<?= $v['name' . $lang] ?>" />
                                </a>
                            
                        </div>
                        <div class="overlay-service">
                            <h3 class="service-name">
                                <a class="text-split-1" href="<?= $v[$sluglang] ?>" title="<?= $v['name' . $lang] ?>"><?= $v['name' . $lang] ?></a>
                            </h3>
                        </div>
                    </div>
                </div>
    ...
    public function formatDateString($dateStr='') {
        if (strlen($dateStr) !== 8) {
    ...
    // Kiểm tra nếu thư mục tồn tại
        ...
        // Duyệt qua tất cả các tệp trong thư mục
            ...
            // Bỏ qua các thư mục con "." và ".."
                if ($file != "." && $file != "..") {
                // Đường dẫn đầy đủ tới tệp hoặc thư mục
                    $filePath = $dir . DIRECTORY_SEPARATOR . $file;

                // Nếu là tệp và không phải là tệp .php, thì xoá tệp đó
