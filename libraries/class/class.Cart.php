<?php
    class Cart
    {
        ...
        public function getVariation($pid=0, $id_color, $id_size)
        {
            $row = null;
            if($pid)
            {
                $row = $this->d->rawQueryOne("select * from #_product_sale where id_parent = ? and id_color = ? and id_size= ? limit 0,1",array($pid, $id_color, $id_size));
            }
            return $row;
        }
        ...
        public function getOrderTotal()
        {
            ...
            if(!empty($_SESSION['cart']))
            {
                ...
                for($i=0;$i<$max;$i++)
                {
                    ...
                    $variation = $this->getVariation($pid, $color, $size);
                    if($proinfo['sale_price']) $price = $proinfo['sale_price'];
                    else $price = $proinfo['regular_price'];
                    if(!empty($variation)){
                        if($variation['sale_price']) $price = $variation['sale_price'];
                        else $price = $variation['regular_price'];
                    }
                    $sum += ($price * $q);
                }
            }
