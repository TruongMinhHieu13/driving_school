        ...
        <div class="card card-primary card-outline text-sm">
            ...
            <div class="card-body">
                ...
                <div class="form-group-category row">
                    ...
                    <?php } ?>
                    <?php if (isset($config['newsletter'][$type]['date']) && $config['newsletter'][$type]['date'] == true) { ?>
                        <div class="form-group col-md-4">
                            <label for="date">Ngày tổ chức:</label>
                            <input type="date" class="form-control text-sm" name="data[date]" id="date" placeholder="Ngày tổ chức" value="<?= (!empty($flash->has('date'))) ? $flash->get('date') : @$item['date'] ?>" required>
                        </div>
                    <?php } ?>
                    <?php if (isset($config['newsletter'][$type]['time']) && $config['newsletter'][$type]['time'] == true) { ?>
                        <div class="form-group col-md-4">
                            <label for="time">Thời gian:</label>
                            <input type="datetime-local" class="form-control text-sm" name="data[time]" id="time" placeholder="" value="<?= (!empty($flash->has('time'))) ? $flash->get('time') : @$item['time'] ?>" required>
                        </div>
                    <?php } ?>
                    <?php if (isset($config['newsletter'][$type]['quantity']) && $config['newsletter'][$type]['quantity'] == true) { ?>
                        <div class="form-group col-md-4">
                            <label for="quantity">Số lượng khách :</label>
                            <input type="number" class="form-control text-sm" name="data[quantity]" id="quantity" placeholder="Số lượng khách " value="<?= (!empty($flash->has('quantity'))) ? $flash->get('quantity') : @$item['quantity'] ?>" required>
                        </div>
                    <?php } ?>
                    ...
                    <?php if (isset($config['newsletter'][$type]['confirm_status']) && count($config['newsletter'][$type]['confirm_status']) > 0) { ?>
                        <div class="form-group col-md-4">
                            <label for="confirm_status"><?=tinhtrang?>:</label>
