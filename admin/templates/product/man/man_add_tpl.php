...
$linkUploadExcel = "index.php?com=import&act=uploadExcel&type=" . $type . "&id=".$item['id'];
...
(isset($config['product'][$type]['color_image']) && $config['product'][$type]['color_image'] == true) ? $configImageColor = true : $configImageColor = false;
(isset($config['product'][$type]['color']) && $config['product'][$type]['color'] == true) ? $configColor = true : $configColor = false;
(isset($config['product'][$type]['size']) && $config['product'][$type]['size'] == true) ? $configSize = true : $configSize = false;
...
<section class="content">
    <form class="validation-form" novalidate method="post" action="<?= $linkSave ?>" enctype="multipart/form-data">
        ...
        <div class="row">
                        ...
                        <?php if (isset($config['product'][$type]['file']) && $config['product'][$type]['file'] == true) { ?>
                            <div class="form-group">
                                <div class="upload-file mb-2">
                                    ...
                                    <?php } ?>
                                    <strong class="d-block text-sm"><?= $config['product'][$type]['file_type'] ?></strong>
                                </div>
                            </div>
                        <?php } ?>
                                ...
                                <div class="tab-content" id="custom-tabs-three-tabContent-lang">
                                    <?php foreach ($config['website']['lang'] as $k => $v) { ?>
                                        <div class="tab-pane fade show <?= ($k == 'vi') ? 'active' : '' ?>" id="tabs-lang-<?= $k ?>" role="tabpanel" aria-labelledby="tabs-lang">
                                            ...
                                            <?php } ?>
                                            <?php if (isset($config['product'][$type]['information']) && $config['product'][$type]['information'] == true) { ?>
                                                <div class="form-group">
                                                    <label for="datasheet">Datasheet:</label>
                                                    <textarea class="form-control for-seo text-sm form-control-ckeditor" name="data[datasheet]" id="datasheet" rows="5" placeholder="Datasheet"><?= $func->decodeHtmlChars($flash->get('datasheet')) ?: $func->decodeHtmlChars(@$item['datasheet']) ?></textarea>
                                                </div>
                                                <div class="form-group">
                                                    <label for="user_manual">User manual:</label>
                                                    <textarea class="form-control for-seo text-sm form-control-ckeditor" name="data[user_manual]" id="user_manual" rows="5" placeholder="User manual"><?= $func->decodeHtmlChars($flash->get('user_manual')) ?: $func->decodeHtmlChars(@$item['user_manual']) ?></textarea>
                                                </div>
                                                <div class="form-group">
                                                    <label for="warranty_condition">Warranty condition:</label>
                                                    <textarea class="form-control for-seo text-sm form-control-ckeditor" name="data[warranty_condition]" id="warranty_condition" rows="5" placeholder="Warranty condition"><?= $func->decodeHtmlChars($flash->get('warranty_condition')) ?: $func->decodeHtmlChars(@$item['warranty_condition']) ?></textarea>
                                                </div>
                                            <?php } ?>
                                        </div>
                                    <?php } ?>
            ...
            <div class="<?= $colRight ?>">
                ...
                <?php if (isset($config['product'][$type]['icon']) && $config['product'][$type]['icon'] == true) { ?>
                    <div class="card card-primary card-outline text-sm">
                        <div class="card-header">
                            <h3 class="card-title"><?= hinhanh ?> <?= $config['product'][$type]['title_main'] ?> 2</h3>
                            <div class="card-tools">
                                <button type="button" class="btn btn-tool" data-card-widget="collapse"><i class="fas fa-minus"></i></button>
                            </div>
                        </div>
        ...
        <?php if(isset($config['product'][$type]['color']) && $config['product'][$type]['color'] == true && isset($config['product'][$type]['size']) && $config['product'][$type]['size'] == true) { ?>
            <div class="card card-primary card-outline text-sm">
                <div class="card-header">
                    <h3 class="card-title">Danh sách phân loại hàng</h3>
                </div>
                <div class="card-body">
                    <div class="row full-SumoSelect">
                    ...
                    <div class="variation-model-table variation-model-wrap <?=!empty($rowVariation) ? 'show' : ''?>">
                        <div class="variation-model-table-header">
                            <div class="variation-model-table-cell">
                                <div class="table-cell-content header-cell-content cell-label">Màu</div>
                                <div class="table-cell-content header-cell-content cell-label">Size</div>
                                <div class="table-cell-content header-cell-content">Giá bán</div>
                                <div class="table-cell-content header-cell-content">Giá mới</div>
                                <div class="table-cell-content header-cell-content cell-option">Hiển thị</div>
                            </div>
                        </div>
                        <div class="variation-model-table-body">
                            <?php if(!empty($rowVariation)){ ?>
                                <?php foreach ($rowVariation as $key => $value) { ?>
                                    <div class="variation-model-table-cell row-price variation-model-table-cell-<?=$value['id_color']?>-<?=$value['id_size']?> variation-model-table-cell-color-<?=$value['id_color']?> variation-model-table-cell-size-<?=$value['id_size']?>">
                                        <div class="table-cell-content body-cell-content cell-label"><?=$value['name_color']?></div>
                                        <div class="table-cell-content body-cell-content cell-label"><?=$value['name_size']?></div>
                                        <div class="table-cell-content body-cell-content">
                                            <input type="text" class="form-control format-price regular_price text-sm" name="dataVariation[<?=$value['id_color']?>][<?=$value['id_size']?>][regular_price]"  placeholder="Giá bán" value="<?=@$value['regular_price']?>">
                                        </div>
                                        <div class="table-cell-content body-cell-content">
                                            <input type="text" class="form-control format-price sale_price text-sm" name="dataVariation[<?=$value['id_color']?>][<?=$value['id_size']?>][sale_price]" placeholder="Giá mới" value="<?=@$value['sale_price']?>">
                                        </div>
                                        <div class="table-cell-content body-cell-content cell-option">
                                            <div class="custom-control custom-checkbox d-inline-block align-middle">
                                                <input type="checkbox" class="custom-control-input <?=$value['id_color']?>-<?=$value['id_size']?>-checkbox" name="dataVariation[<?=$value['id_color']?>][<?=$value['id_size']?>][status]" id="<?=$value['id_color']?>-<?=$value['id_size']?>-checkbox" <?=$value['status'] ? 'checked' : ''?> value="1">
                                                <label for="<?=$value['id_color']?>-<?=$value['id_size']?>-checkbox" class="custom-control-label"></label>
                                            </div>
                                        </div>
                                    </div>
                                <?php } ?>
        ...
        <div class="card-footer text-sm">
            ...
            <button type="reset" class="btn btn-sm bg-gradient-secondary"><i class="fas fa-redo mr-2"></i><?= lamlai ?></button>
            <a class="btn btn-sm bg-gradient-danger" href="<?= $linkMan ?>" title="<?= thoat ?>"><i class="fas fa-sign-out-alt mr-2"></i><?= thoat ?></a>
            <input type="hidden" name="id" class="productId" value="<?= (isset($item['id']) && $item['id'] > 0) ? $item['id'] : '' ?>">
        </div>
    </form>
</section>