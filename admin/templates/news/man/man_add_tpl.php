...
/* None category */
if ( isset($config['news'][$type]['hidden-category']) && $config['news'][$type]['hidden-category'] == true) {
    $noneCatagory = "d-none";
} else {
    $noneCatagory = "";
}
...
/* None col right*/
if ((isset($config['news'][$type]['hidden-category']) && $config['news'][$type]['hidden-category'] == true) && (isset($config['news'][$type]['images']) && $config['news'][$type]['images'] == false)) {
    $colLeft = "col-12";
    $colRight = "d-none";
} else {
    $colLeft = "col-xl-8";
        ...
        <div class="row">
                                ...
                                <div class="upload-file mb-2">
                                    ...
                                    <label class="upload-file-label mb-2" for="file_attach">
                                        <div class="custom-file my-custom-file">
                                            <input type="file" class="custom-file-input" name="file_attach" id="file_attach" lang="vi"  <?=(isset($item['file_attach']) && $item['file_attach'] != '') ? '' : 'required'?>>
                                        ...
                                        <div class="tab-pane fade show <?= ($k == 'vi') ? 'active' : '' ?>" id="tabs-lang-<?= $k ?>" role="tabpanel" aria-labelledby="tabs-lang">
                                            ...
                                            <?php if (isset($config['news'][$type]['link']) && $config['news'][$type]['link'] == true) { ?>
                                                <div class="form-group">
                                                    ...
                                                    <input type="text"  class="form-control for-seo text-sm" name="data[link]" id="link" rows="1" placeholder="Link" value="<?= $func->decodeHtmlChars($flash->get('link')) ?: $func->decodeHtmlChars(@$item['link']) ?>">
                                                </div>
                                            <?php } ?>
                                            <?php if (isset($config['news'][$type]['desc2']) && $config['news'][$type]['desc2'] == true) { ?>
                                                <div class="form-group">
                                                    <label for="desc2<?= $k ?>"><?=mota?> ngắn (<?= $k ?>):</label>
                                                    <textarea class="form-control for-seo text-sm <?= (isset($config['news'][$type]['desc2_cke']) && $config['news'][$type]['desc2_cke'] == true) ? 'form-control-ckeditor' : '' ?>" name="data[desc2<?= $k ?>]" id="desc2<?= $k ?>" rows="5" placeholder="<?=mota?> ngắn (<?= $k ?>)"><?= $func->decodeHtmlChars($flash->get('desc2' . $k)) ?: $func->decodeHtmlChars(@$item['desc2' . $k]) ?></textarea>
                                                </div>
                                            <?php } ?>
                                            <?php if ($type == 'lich-hoc') { ?>
                                                <div class="form-group">
                                                    <label for="morning<?= $k ?>">Lịch học sáng (<?= $k ?>):</label>
                                                    <textarea class="form-control for-seo text-sm" name="data[morning<?= $k ?>]" id="morning<?= $k ?>" rows="1" placeholder="Lịch học sáng (<?= $k ?>)"><?= $func->decodeHtmlChars($flash->get('morning' . $k)) ?: $func->decodeHtmlChars(@$item['morning' . $k]) ?></textarea>
                                                </div>
                                                <div class="form-group">
                                                    <label for="noon<?= $k ?>">Lịch học trưa (<?= $k ?>):</label>
                                                    <textarea class="form-control for-seo text-sm" name="data[noon<?= $k ?>]" id="noon<?= $k ?>" rows="1" placeholder="Lịch học trưa (<?= $k ?>)"><?= $func->decodeHtmlChars($flash->get('noon' . $k)) ?: $func->decodeHtmlChars(@$item['noon' . $k]) ?></textarea>
                                                </div>
                                                <div class="form-group">
                                                    <label for="afternoon<?= $k ?>">Lịch học chiều (<?= $k ?>):</label>
                                                    <textarea class="form-control for-seo text-sm" name="data[afternoon<?= $k ?>]" id="afternoon<?= $k ?>" rows="1" placeholder="Lịch học chiều (<?= $k ?>)"><?= $func->decodeHtmlChars($flash->get('afternoon' . $k)) ?: $func->decodeHtmlChars(@$item['afternoon' . $k]) ?></textarea>
                                                </div>
                                            <?php } ?>

                                            <?php if ($type == 'thong-ke') { ?>
                                                <div class="form-group">
                                                    <label for="quanlity">Số lượng:</label>
                                                    <input type="number" class="form-control text-sm" name="data[quanlity]" id="quanlity" rows="1" placeholder="Số lượng" value="<?= $func->decodeHtmlChars($flash->get('quanlity')) ?: $func->decodeHtmlChars(@$item['quanlity']) ?>">
                                                </div>
                                                <div class="form-group">
                                                    <label for="unit">Đơn vị:</label>
                                                    <input type="text" class="form-control text-sm" name="data[unit]" id="unit" placeholder="Đơn vị" value="<?= $func->decodeHtmlChars($flash->get('unit')) ?: $func->decodeHtmlChars(@$item['unit']) ?>">
                                                </div>
                                                 <?php $status_array = (!empty($item['status'])) ? explode(',', $item['status']) : array(); ?>
                                                <div class="form-group">
                                                    <label for="countup-checkbox" class="d-inline-block align-middle mb-0 mr-2">Chạy số:</label>
                                                    <div class="custom-control custom-checkbox d-inline-block align-middle">
                                                        <input type="checkbox" class="custom-control-input countup-checkbox" name="status[countup]" id="countup-checkbox" value="countup" <?= (empty($status_array) && empty($item['id']) ? 'checked' : in_array('countup', $status_array)) ? 'checked' : '' ?>>
                                                        <label for="countup-checkbox" class="custom-control-label"></label>
                                                    </div>
                                                </div>
                                                <p class="text-danger font-weight-bold">Không cần số tự động chạy thì không nhập ô số lượng</p>
                                            <?php } ?>
            ...
            <div class="<?= $colRight ?>">
                <?php if (
                    ...
                    <div class="card card-primary card-outline text-sm <?=$noneCatagory?>">
                        <div class="card-header">
