...
if (!isset($config['order']['excelall']) || $config['order']['excelall'] == false) $func->transfer(trangkhongtontai, "index.php", false);
...
$PHPExcel->setActiveSheetIndex(0)->setCellValue('B6', madonhang);
$PHPExcel->setActiveSheetIndex(0)->setCellValue('C6', ngaydat);
$PHPExcel->setActiveSheetIndex(0)->setCellValue('D6', tinhtrang);
$PHPExcel->setActiveSheetIndex(0)->setCellValue('E6', hinhthucthanhtoan);
$PHPExcel->setActiveSheetIndex(0)->setCellValue('F6', hoten);
$PHPExcel->setActiveSheetIndex(0)->setCellValue('G6', dienthoai);
$PHPExcel->setActiveSheetIndex(0)->setCellValue('H6', 'Email');
$PHPExcel->setActiveSheetIndex(0)->setCellValue('I6', diachi);
$PHPExcel->setActiveSheetIndex(0)->setCellValue('J6', tamtinh);
$PHPExcel->setActiveSheetIndex(0)->setCellValue('K6', phivanchuyen);
$PHPExcel->setActiveSheetIndex(0)->setCellValue('L6', tonggiatridonhang);
...
for ($i = 0; $i < count($orders); $i++) {
    ...
    $PHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('B' . $position, $func->decodeHtmlChars($orders[$i]['code']))
        ->setCellValue('C' . $position, date('H:i A d-m-Y', $orders[$i]['date_created']))
        ->setCellValue('D' . $position, $func->decodeHtmlChars($order_status_info['namevi']))
        ->setCellValue('E' . $position, $func->decodeHtmlChars($order_payment['namevi']))
        ->setCellValue('F' . $position, $func->decodeHtmlChars($orders[$i]['fullname']))
        ->setCellValue('G' . $position, $func->decodeHtmlChars($orders[$i]['phone']))
        ->setCellValue('H' . $position, $func->decodeHtmlChars($orders[$i]['email']))
        ->setCellValue('I' . $position, $func->decodeHtmlChars($orders[$i]['address']))
        ->setCellValue('J' . $position, $func->formatMoney($orders[$i]['temp_price']))
        ->setCellValue('K' . $position, $func->decodeHtmlChars($ship_price))
...
/* Rename title */
$PHPExcel->getActiveSheet()->setTitle(danhsachdonhang);
