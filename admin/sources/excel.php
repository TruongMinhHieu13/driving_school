...
if (!isset($config['order']['excel']) || $config['order']['excel'] == false) $func->transfer(trangkhongtontai, "index.php", false);
...
if (empty($id_order)) $func->transfer(dulieukhongcothuc, "index.php?com=order&act=man", false);
...
$PHPExcel->setActiveSheetIndex(0)->setCellValue('A4', diachi . ': ' . $optsetting['address']);
$PHPExcel->setActiveSheetIndex(0)->setCellValue('A6', hoten . ': ' . @$order_detail['fullname']);
$PHPExcel->setActiveSheetIndex(0)->setCellValue('A7', dienthoai . ': ' . @$order_detail['phone']);
$PHPExcel->setActiveSheetIndex(0)->setCellValue('A8', diachi . ': ' . @$order_detail['address']);
$PHPExcel->setActiveSheetIndex(0)->setCellValue('D6', madonhang . ': ' . $code);
$PHPExcel->setActiveSheetIndex(0)->setCellValue('D7', ngaydat . ': ' . date('H:i A d-m-Y', @$order_detail['date_created']));
$PHPExcel->setActiveSheetIndex(0)->setCellValue('D8', tinhtrang . ': ' . $order_status_info['namevi']);
$PHPExcel->setActiveSheetIndex(0)->setCellValue('A10', 'STT');
$PHPExcel->setActiveSheetIndex(0)->setCellValue('B10', sanpham);
$PHPExcel->setActiveSheetIndex(0)->setCellValue('C10', soluong);
$PHPExcel->setActiveSheetIndex(0)->setCellValue('D10', gia);
$PHPExcel->setActiveSheetIndex(0)->setCellValue('E10', giamoi);
$PHPExcel->setActiveSheetIndex(0)->setCellValue('F10', tamtinh);
...
for ($i = 0; $i < count($items); $i++) {
    ...
    $name_pro = @$items[$i]['name'];
    if (!empty($items[$i]['size']) || !empty($items[$i]['color'])) {
        $name_pro .= ' (';
        if (!empty($items[$i]['color'])) {
            $name_pro .= $items[$i]['color'];
        }
        if (!empty($items[$i]['size'])) {
            $name_pro .= '-' . $items[$i]['size'];
        }
        $name_pro .= ')';
    }
    ...
    $PHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('B' . $position, $name_pro)
...
if ($config['order']['ship']) {
    ...
    $PHPExcel->setActiveSheetIndex(0)->setCellValue('A' . $position, tamtinh);
...
if ($config['order']['ship']) {
    $PHPExcel->setActiveSheetIndex(0)->mergeCells('A' . $position . ':E' . $position);
    $PHPExcel->setActiveSheetIndex(0)->setCellValue('A' . $position, phivanchuyen);
...
/* Tổng tiền */
$PHPExcel->setActiveSheetIndex(0)->mergeCells('A' . $position . ':E' . $position);
$PHPExcel->setActiveSheetIndex(0)->setCellValue('A' . $position, tonggiatridonhang);
