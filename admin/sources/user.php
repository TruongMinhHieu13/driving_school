...
{
    global $d, $langadmin, $func, $curPage, $items, $paging, $config, $type;
    ...
    $startpoint = ($curPage * $perPage) - $perPage;
    $limit = " limit " . $startpoint . "," . $perPage;
    $sql = "select * from #_member where id <> 0 and chucnang = '' $where order by numb,id desc $limit";
    $items = $d->rawQuery($sql);
    $sqlNum = "select count(*) as 'num' from #_member where id <> 0 and chucnang = '' $where order by numb,id desc";
    $count = $d->rawQueryOne($sqlNum);
    $total = (!empty($count)) ? $count['num'] : 0;
...
function editMember()
{
    global $d, $langadmin, $func, $curPage, $item, $personalImage;
    ...
    if (empty($id)) {
        ...
        $personalImage = $d->rawQuery("select photo, id from #_gallery where id_parent = ? order by numb,id desc", array($item['id']));
