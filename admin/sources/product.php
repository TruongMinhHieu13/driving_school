...
function editMan()
{
    global $d, $langadmin, $func, $strUrl, $curPage, $item, $gallery, $type, $com, $act, $dataVariationColor, $dataVariationSize, $rowVariation, $rowSize;
    ...
    if (empty($id)) {
        ...
        if (empty($item)) {
            ...
            $rowVariation = $d->rawQuery("select A.id, A.id_color, A.id_size, A.regular_price, A.sale_price, A.status, A.photo, B.namevi as name_color, C.namevi as name_size from #_product_sale as A LEFT JOIN #_color as B ON A.id_color = B.id LEFT JOIN #_size as C ON A.id_size = C.id where id_parent = ? order by A.id_color, A.id desc", array($item['id']));

            $rowSize = (!empty($rowVariation)) ? $func->joinColsArray($rowVariation, 'id_size') : array();
...
function saveMan()
{
    ...
    $dataVariation = (!empty($_POST['dataVariation'])) ? $_POST['dataVariation'] : null;
    ...
    if ($id && $act != 'save_copy') {
        ...
        if ($d->update('product', $data)) {
            ...
            if ($func->hasFile("file_attach")) {
                ...
                if ($file_attach = $func->uploadImage("file_attach", $config['product'][$type]['file_type'], UPLOAD_FILE, $file_name)) {
                    $row = $d->rawQueryOne("select id, file_attach from #_product where type = ? limit 0,1", array($type));
            ...
            if (!empty($config['product'][$type]['color']) || !empty($config['product'][$type]['size'])) {
                ...
                if (!empty($dataSale1['data']) || !empty($dataSale2['data'])) {
                    ...
                    if (!empty($dataSale1['data'])) {
                        $colSale1 = $dataSale1['id'];
                        $whereDeleteSale .= implode(" AND ", array_map(function ($v_sale1) use ($colSale1) {
                            return " $colSale1 <> '$v_sale1'";
                        }, $dataSale1['data']));
                    }
                    ...
                    if (!empty($dataSale2['data'])) {
                        ...
                        $colSale2 = $dataSale2['id'];
                        $whereDeleteSale .= implode(" AND ", array_map(function ($v_sale2) use ($colSale2) {
                            return " $colSale2 <> '$v_sale2'";
                        }, $dataSale2['data']));
                    }


                    $whereDeleteSale .= ")";
                    $rowDeleteSale = $d->rawQuery("select id, photo from #_product_sale where id_parent = ? $whereDeleteSale", array($id));

                    if (!empty($rowDeleteSale)) {
                        foreach ($rowDeleteSale as $v) {
                            $func->deleteFile(UPLOAD_PRODUCT . $v['photo']);
                            $d->rawQuery("delete from #_product_sale where id_parent = ? and id = ?", array($id, $v['id']));
                        }
                    }

                    $rowProductSale = $d->rawQuery("select id, id_color, id_size from #_product_sale where id_parent = ?", array($id));
                    ...
                    if (!empty($dataSale1['data'])) {
                        foreach ($dataSale1['data'] as $v_sale1) {
                            ...
                            $colSale1 = $dataSale1['id'];
                            $dataSale[$colSale1] = $v_sale1;

                            if ($func->hasFile("dataImage" . $v_sale1)) {
                                $photoUpdate = array();
                                $file_name = $func->uploadName($_FILES["dataImage" . $v_sale1]["name"]);

                                if ($photo = $func->uploadImage("dataImage" . $v_sale1, $config['product'][$type]['img_type'], UPLOAD_PRODUCT, $file_name)) {
                                    $row = $d->rawQueryOne("select id, photo from #_product_sale where id_parent = ? and id_color = ? limit 0,1", array($id, $v_sale1));

                                    if (!empty($row)) {
                                        $func->deleteFile(UPLOAD_PRODUCT . $row['photo']);
                                    }

                                    $dataSale['photo'] = $photo;
                                }
                            }
                            ...
                            if (!empty($dataSale2['data'])) {
                                foreach ($dataSale2['data'] as $v_sale2) {
                                    $colSale2 = $dataSale2['id'];
                                    $dataSale['regular_price'] = (isset($dataVariation[$v_sale1][$v_sale2]['regular_price']) && $dataVariation[$v_sale1][$v_sale2]['regular_price'] != '') ? str_replace(",", "", $dataVariation[$v_sale1][$v_sale2]['regular_price']) : 0;
                                    $dataSale['sale_price'] = (isset($dataVariation[$v_sale1][$v_sale2]['sale_price']) && $dataVariation[$v_sale1][$v_sale2]['sale_price'] != '') ? str_replace(",", "", $dataVariation[$v_sale1][$v_sale2]['sale_price']) : 0;
                                    $dataSale['status'] = (isset($dataVariation[$v_sale1][$v_sale2]['status']) && $dataVariation[$v_sale1][$v_sale2]['status'] != '') ? str_replace(",", "", $dataVariation[$v_sale1][$v_sale2]['status']) : 0;
                                    $dataSale[$colSale2] = $v_sale2;

                                    $idProductSale = $func->checkProductSale($rowProductSale, $v_sale1, $v_sale2);
                                    if ($idProductSale > 0) {
                                        $d->where('id', $idProductSale);
                                        $d->update('product_sale', $dataSale);
                                    } else {
                                        $d->insert('product_sale', $dataSale);
                                    }
                                }
                            } else {
                                $dataSale['regular_price'] = (isset($dataVariation[$v_sale1][0]['regular_price']) && $dataVariation[$v_sale1][0]['regular_price'] != '') ? str_replace(",", "", $dataVariation[$v_sale1][0]['regular_price']) : 0;
                                $dataSale['sale_price'] = (isset($dataVariation[$v_sale1][0]['sale_price']) && $dataVariation[$v_sale1][0]['sale_price'] != '') ? str_replace(",", "", $dataVariation[$v_sale1][0]['sale_price']) : 0;
                                $dataSale['status'] = (isset($dataVariation[$v_sale1][0]['status']) && $dataVariation[$v_sale1][0]['status'] != '') ? str_replace(",", "", $dataVariation[$v_sale1][0]['status']) : 0;

                                $idProductSale = $func->checkProductSale($rowProductSale, $v_sale1, 0);
                                if ($idProductSale > 0) {
                                    $d->where('id', $idProductSale);
                                    $d->update('product_sale', $dataSale);
                                } else {
                        ...
                        foreach ($dataSale2['data'] as $v_sale2) {
                            ...
                            $dataSale['regular_price'] = (isset($dataVariation[0][$v_sale2]['regular_price']) && $dataVariation[0][$v_sale2]['regular_price'] != '') ? str_replace(",", "", $dataVariation[0][$v_sale2]['regular_price']) : 0;
                            $dataSale['sale_price'] = (isset($dataVariation[0][$v_sale2]['sale_price']) && $dataVariation[0][$v_sale2]['sale_price'] != '') ? str_replace(",", "", $dataVariation[0][$v_sale2]['sale_price']) : 0;
                            $dataSale['status'] = (isset($dataVariation[0][$v_sale2]['status']) && $dataVariation[0][$v_sale2]['status'] != '') ? str_replace(",", "", $dataVariation[0][$v_sale2]['status']) : 0;

                            $idProductSale = $func->checkProductSale($rowProductSale, 0, $v_sale2);
                            if ($idProductSale > 0) {
                                $d->where('id', $idProductSale);
                                $d->update('product_sale', $dataSale);
                            } else {
                    ...
                    $rowDeleteSale = $d->rawQuery("select id, photo from #_product_sale where id_parent = ?", array($id));
                ...
                $dataJson = array('type' => 'product_sale', 'id_product' => $id);
        ...
        if ($d->insert('product', $data)) {
            ...
            if (!empty($config['product'][$type]['color']) || !empty($config['product'][$type]['size'])) {
                ...
                $dataJson = array('type' => 'product_sale', 'id_product' => $id_insert);
                $func->writeJson($dataJson);
            }
...
function deleteMan()
{
    ...
    if ($id) {
        ...
        if (!empty($row)) {
            ...
            /* Xóa Sale */
            $rowProductSale = $d->rawQuery("select id, photo from #_product_sale where id_parent = ?", array($id));

            if (!empty($rowProductSale)) {
                foreach ($rowProductSale as $v) {
                    $func->deleteFile(UPLOAD_PRODUCT . $v['photo']);
                }
                $d->rawQuery("delete from #_product_sale where id_parent = ?", array($id));
            }
